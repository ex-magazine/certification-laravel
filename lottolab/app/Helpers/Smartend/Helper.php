...
namespace App\Helpers\Smartend;

use App;
use App\Models\Smartend\SmartendAnalyticsPage;
use App\Models\Smartend\SmartendAnalyticsVisitor;
use App\Models\Smartend\SmartendBanner;
use App\Models\Smartend\SmartendCountry;
use App\Models\Smartend\SmartendEvent;
use App\Models\Smartend\SmartendMenu;
...
use App\Models\Smartend\SmartendLanguage;
...
class Helper
{
    ...
    {
        $WebmasterSetting = SmartendWebmasterSetting::find(1);
        return $WebmasterSetting->$var;
    }

    static function GeneralSiteSettings($var)
    {
        $Setting = SmartendSetting::find(1);
        return $Setting->$var;
    }
    ...
    static function eventsAlerts()
    {
        if (@Auth::user()->permissionsGroup->view_status) {
            $Events = SmartendEvent::where('created_by', '=', Auth::user()->id)->where('start_date', '>=', date('Y-m-d H:i:s'))->orderby('start_date', 'asc')->limit(10)->get();
        } else {
            $Events = SmartendEvent::where('start_date', '>=', date('Y-m-d H:i:s'))->orderby('start_date', 'asc')->limit(10)->get();
        }
        return $Events;
    }
    ...
    static function webmailsAlerts()
    {
        ...
        if (@Auth::user()->permissionsGroup->view_status) {
            $Webmails = SmartendWebmail::where('created_by', '=', Auth::user()->id)->orderby('id', 'desc')->where('status', '=',
                0)
                ->where('cat_id', '=', 0)->limit(4)->get();
        } else {
            $Webmails = SmartendWebmail::orderby('id', 'desc')->where('status', '=', 0)
                ->where('cat_id', '=', 0)->limit(4)->get();
        }
    ...
    static function webmailsNewCount()
    {
        ...
        if (@Auth::user()->permissionsGroup->view_status) {
            ...
            $Webmails = SmartendWebmail::orderby('id', 'desc')->where('status', '=', 0)->where('cat_id', '=', 0)->get();
        }
    ...
    {
        return SmartendBanner::where('section_id', $BannersSettingsId)->where('status', 1)->orderby('row_no', 'asc')->get();
    }

    // Menu array List
    static function MenuList($GroupId)
    {
        return SmartendMenu::where('father_id', $GroupId)->where('status', 1)->orderby('row_no', 'asc')->get();
    }
    ...
    static function SaveVisitorInfo($PageTitle)
    {
        if (env('GEOIP_STATUS', false)) {
            ...
            $SavedVisitor = SmartendAnalyticsVisitor::where('ip', '=', $visitor_ip)->where('date', '=', date('Y-m-d'))->first();
            if (empty($SavedVisitor)) {

                // New to analyticsVisitors
                try {
                    $visitor_local_ip_details = SmartendAnalyticsVisitor::where('ip', $visitor_ip)->first();
                    if (!empty($visitor_local_ip_details)) {
                        ...
                        if ($visitor_country_code != "") {
                            $v_country = SmartendCountry::where('code', '=', $visitor_country_code)->first();
                            if (!empty($v_country)) {
                ...
                $Visitor = new SmartendAnalyticsVisitor;
                ...
                $VisitedPage = new SmartendAnalyticsPage;
                ...
                // Check if page saved
                $Savedpage = SmartendAnalyticsPage::where('visitor_id', '=', $SavedVisitor->id)->where('ip', '=',
    ...
    static function URLSlug($url, $type = "", $id = 0, $num = 0)
    {
        ...
        if ($type == "section" && $id > 0) {
            ...
            if (count(Helper::languagesList()) > 0) {
                ...
                foreach (Helper::languagesList() as $ActiveLanguage) {
                    if ($i == 0) {
                        $check_WebmasterSection = SmartendWebmasterSection::where([['seo_url_slug_' . $ActiveLanguage->code, $seo_url_slug], ['id', '!=', $id]]);
                    } else {
            ...
            if (count(Helper::languagesList()) > 0) {
                ...
                foreach (Helper::languagesList() as $ActiveLanguage) {
                    if ($i == 0) {
                        $check_WebmasterSection = SmartendWebmasterSection::where('seo_url_slug_' . $ActiveLanguage->code, $seo_url_slug);
                    } else {
        ...
        if ($type == "category" && $id > 0) {
            ...
            if (count(Helper::languagesList()) > 0) {
                ...
                foreach (Helper::languagesList() as $ActiveLanguage) {
                    if ($i == 0) {
                        $check_Section = SmartendSection::where([['seo_url_slug_' . $ActiveLanguage->code, $seo_url_slug], ['id', '!=', $id]]);
                    } else {
            ...
            if (count(Helper::languagesList()) > 0) {
                ...
                foreach (Helper::languagesList() as $ActiveLanguage) {
                    if ($i == 0) {
                        $check_Section = SmartendSection::where('seo_url_slug_' . $ActiveLanguage->code, $seo_url_slug);
                    } else {
        ...
        if ($type == "topic" && $id > 0) {
            ...
            if (count(Helper::languagesList()) > 0) {
                ...
                foreach (Helper::languagesList() as $ActiveLanguage) {
                    if ($i == 0) {
                        $check_Topic = SmartendTopic::where([['seo_url_slug_' . $ActiveLanguage->code, $seo_url_slug], ['id', '!=', $id]]);
                    } else {
            ...
            if (count(Helper::languagesList()) > 0) {
                ...
                foreach (Helper::languagesList() as $ActiveLanguage) {
                    if ($i == 0) {
                        $check_Topic = SmartendTopic::where('seo_url_slug_' . $ActiveLanguage->code, $seo_url_slug);
                    } else {
    ...
    static function currentLanguage()
    {
        ...
        $Language = SmartendLanguage::where("code", $locale)->first();
        if (empty($Language)) {
            $Language = SmartendLanguage::where("code", env('DEFAULT_LANGUAGE', 'en'))->first();
        }
        return $Language;
    }
    ...
    {
        return SmartendLanguage::where("code", $code)->first();
    }

    static function languagesList()
    {
        return SmartendLanguage::where("status", true)->orderby('id', 'asc')->get();
    }
    ...
    static function sectionURL($id, $lang = "")
    {
        ...
        try {
            ...
            $WebmasterSection = SmartendWebmasterSection::find($id);
    ...
    static function categoryURL($id, $lang = "")
    {
        ...
        try {
            ...
            $Category = SmartendSection::find($id);
            if (!empty($Category)) {

...
namespace App\Http\Controllers\Smartend;

use Illuminate\Support\Facades\File;
use App;
use App\Http\Controllers\Controller;
use App\Models\Smartend\SmartendBanner;
use App\Mail\NotificationEmail;
use App\Models\Smartend\SmartendComment;
use App\Models\Smartend\SmartendContact;
use App\Models\Smartend\SmartendSection;
use App\Models\Smartend\SmartendSetting;
use App\Models\Smartend\SmartendTopic;
use App\Models\Smartend\SmartendTopicCategory;
use App\Models\Smartend\SmartendTopicField;
use App\Models\Smartend\SmartendUser;
use App\Models\Smartend\SmartendWebmail;
use App\Models\Smartend\SmartendWebmasterSection;
use App\Models\Smartend\SmartendWebmasterSetting;
...
{ 
    ...
    public function SEOByLang($lang = "", $seo_url_slug = 0)
    {
        ...
        $WebmasterSettings = SmartendWebmasterSetting::find(1);
        ...
        $WebmasterSection1 = SmartendWebmasterSection::where($Current_Slug, $seo_url_slug)->orwhere($Default_Slug, $seo_url_slug)->first();
        if (!empty($WebmasterSection1)) {
            // MAIN SITE SECTION
            $section = $WebmasterSection1->id;
            return $this->topics($section, 0);
        } else {
            $WebmasterSection2 = SmartendWebmasterSection::where($Current_Title, $seo_url_slug)->orwhere($Default_Title, $seo_url_slug)->first();
            ...
            if (!empty($WebmasterSection2)) {
                ...
                $Section = SmartendSection::where('status', 1)->where($Current_Slug, $seo_url_slug)->orwhere($Default_Slug, $seo_url_slug)->first();
                if (empty($Section)) {
                ...
                if (!empty($Section)) {
                    ...
                    $Topic = SmartendTopic::where('status', 1)->where($Current_Slug, $seo_url_slug)->orwhere($Default_Slug, $seo_url_slug)->first();
                    if (empty($Topic)) {
                    ...
                    if (!empty($Topic)) {
                        ...
                        $WebmasterSection = SmartendWebmasterSection::find($section_id);
    ...
    public function HomePageByLang($lang = "")
    {
        ...
        $WebsiteSettings = SmartendSetting::find(1);
       
        // Home topics
        $HomeTopics = SmartendTopic::where([['status', 1], ['webmaster_id', $WebmasterSettings->home_content1_section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $WebmasterSettings->home_content1_section_id], ['expire_date', null]])->orderby('row_no', env("FRONTEND_TOPICS_ORDER", "asc"))->limit(12)->get();
        // Home photos
        $HomePhotos = SmartendTopic::where([['status', 1], ['webmaster_id', $WebmasterSettings->home_content2_section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $WebmasterSettings->home_content2_section_id], ['expire_date', null]])->orderby('row_no', env("FRONTEND_TOPICS_ORDER", "asc"))->limit(6)->get();
// Home Partners
        $HomePartners = SmartendTopic::where([['status', 1], ['webmaster_id', $WebmasterSettings->home_content3_section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $WebmasterSettings->home_content3_section_id], ['expire_date', null]])->orderby('row_no', env("FRONTEND_TOPICS_ORDER", "asc"))->get();

        // Get Latest News
        $LatestNews = $this->latest_topics($WebmasterSettings->latest_news_section_id);

        // Get Home page slider banners
        $SliderBanners = SmartendBanner::where('section_id', $WebmasterSettings->home_banners_section_id)->where('status',
            1)->orderby('row_no', 'asc')->get();

        // Get Home page Test banners
        $TextBanners = SmartendBanner::where('section_id', $WebmasterSettings->home_text_banners_section_id)->where('status',
            1)->orderby('row_no', 'asc')->get();
        ...
        return view("smartend.frontEnd.home",
    ...
    public function topicsByLang($lang = "", $section = 0, $cat = 0)
    {
        ...
        $Current_Slug = "seo_url_slug_" . @Helper::currentLanguage()->code;
        $Default_Slug = "seo_url_slug_" . env('DEFAULT_LANGUAGE');
        $WebmasterSection = SmartendWebmasterSection::where($Current_Slug, $section)->orwhere($Default_Slug, $section)->first();
        if (empty($WebmasterSection)) {
            // get Webmaster section settings by ID
            $WebmasterSection = SmartendWebmasterSection::find($section);
        }
        if (!empty($WebmasterSection)) {
            ...
            $AllSections = SmartendSection::where('webmaster_id', '=', $WebmasterSection->id)->where('status', 1)->orderby('row_no', 'asc')->get();
            ...
            $CurrentCategory = SmartendSection::find($cat);
            ...
            if (!empty($CurrentCategory)) {
                $category_topics = array();
                $TopicCategories = SmartendTopicCategory::where('section_id', $cat)->get();
                foreach ($TopicCategories as $category) {
                    $category_topics[] = $category->topic_id;
                }
                // update visits
                ...
                $Topics = SmartendTopic::where(function ($q) use ($WebmasterSection) {
                ...
                $TopicsMostViewed = SmartendTopic::where([['webmaster_id', '=', $WebmasterSection->id], ['status', 1], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orWhere([['webmaster_id', '=', $WebmasterSection->id], ['status', 1], ['expire_date', null]])->whereIn('id', $category_topics)->orderby('visits', 'desc')->limit(3)->get();
            } else {
                // Topics if NO Cat_ID
                $Topics = SmartendTopic::where([['webmaster_id', '=', $WebmasterSection->id], ['status',
                ...
                // Get Most Viewed
                $TopicsMostViewed = SmartendTopic::where([['webmaster_id', '=', $WebmasterSection->id], ['status',
            ...
            $SideBanners = SmartendBanner::where('section_id', $WebmasterSettings->side_banners_section_id)->where('status',
                1)->orderby('row_no', 'asc')->get();
    ...
    public function topicByLang($lang = "", $section = 0, $id = 0)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($Topic) && ($Topic->expire_date == '' || ($Topic->expire_date != '' && $Topic->expire_date >= date("Y-m-d")))) {
                ...
                // Get Most Viewed
                $TopicsMostViewed = SmartendTopic::where([['webmaster_id', '=', $WebmasterSection->id], ['status', 1], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['webmaster_id', '=', $WebmasterSection->id], ['status', 1], ['expire_date', null]])->orderby('visits', 'desc')->limit(3)->get();
    ...
    public function userTopicsByLang($lang = "", $id)
    {
        ...
        // get User Details
        $User = SmartendUser::find($id);
        if (!empty($User)) {


            // count topics by Category
            $category_and_topics_count = array();
            $AllSections = SmartendSection::where('status', 1)->orderby('row_no', 'asc')->get();
            ...
            // Get a list of all Category ( for side bar )
            $Categories = SmartendSection::where('father_id', '=',
                '0')->where('status', 1)->orderby('webmaster_id', 'asc')->orderby('row_no', 'asc')->get();
            ...
            // Get Most Viewed
            $TopicsMostViewed = SmartendTopic::where([['created_by', $User->id], ['status', 1], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['created_by', $User->id], ['status', 1], ['expire_date', null]])->orderby('visits', 'desc')->limit(3)->get();
    ...
    public function searchTopics(Request $request)
    {
        ...
        if ($search_word != "") {
            ...
            if (!empty($AllSections)) {
                foreach ($AllSections as $AllSection) {
                    $category_topics = array();
                    $TopicCategories = SmartendTopicCategory::where('section_id', $AllSection->id)->get();
                    foreach ($TopicCategories as $category) {
                        $category_topics[] = $category->topic_id;
                    }
            ...
            // Topics if NO Cat_ID
            $Topics = SmartendTopic::where('title_' . Helper::currentLanguage()->code, 'like', '%' . $search_word . '%')
            ...
            // Get Most Viewed
            $TopicsMostViewed = SmartendTopic::where([['status', 1], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['expire_date', null]])->orderby('visits', 'desc')->limit(3)->get();
    ...
    public function ContactPageByLang($lang = "")
    {
        ...
        $Topic = SmartendTopic::where('status', 1)->find($id);
        ...
        if (!empty($Topic) && ($Topic->expire_date == '' || ($Topic->expire_date != '' && $Topic->expire_date >= date("Y-m-d")))) {
            ...
            // get Webmaster section settings by ID
            $WebmasterSection = SmartendWebmasterSection::find($Topic->webmaster_id);

            if (!empty($WebmasterSection)) {

                // Get current Category Section details
                $CurrentCategory = SmartendSection::find($Topic->section_id);
                // Get a list of all Category ( for side bar )
                $Categories = SmartendSection::where('webmaster_id', '=', $WebmasterSection->id)->where('father_id', '=',
                    '0')->where('status', 1)->orderby('webmaster_id', 'asc')->orderby('row_no', 'asc')->get();
    ...
    public function commentSubmit(Request $request)
    {
        ...
        // Topic details
        $Topic = SmartendTopic::where('status', 1)->find($request->topic_id);
        if (!empty($Topic)) {
    ...
    {
        return SmartendTopic::where([['status', 1], ['webmaster_id', $section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $section_id], ['expire_date', null]])->orderby('row_no', 'desc')->limit($limit)->get();
    }
